...
> 🚨 **モード1（作業計画）とモード2（実装）の間には、必ずユーザーの承認ステップが入ります。** モード1でステアリングファイルを作成したら、ユーザーの明示的な承認を得るまでモード2の実装に進んではいけません（詳細は後述の「ゲート: ユーザー承認」を参照）。
...
**MUST（必須）**:
- **実装開始前に、モード1のステアリングファイルに対するユーザー承認を得ていることを確認する（「ゲート: ユーザー承認」参照）**
